...
import { validateArtifact, ValidationError, type ArtifactByKind } from './validation.js';
...
async function main(): Promise<void> {
  ...
  if (command === 'diff') { requireArgs(positionals, 3, 'diff requires <before.json> and <after.json>.'); const result = diffSnapshots(await artifact(positionals[1]!, root, 'design-system-snapshot'), await artifact(positionals[2]!, root, 'design-system-snapshot')); await output(renderArtifact(result, format), values.output, root); return; }
  if (command === 'impact') { requireArgs(positionals, 2, 'impact requires <diff.json>.'); const config = await loadConfig(root, values.config); const diff = await artifact(positionals[1]!, root, 'design-system-diff'); const ownership = config.ownership ? await createCodeownersResolver(path.resolve(root, config.ownership.codeowners)) : undefined; const report = await analyzeImpact({ root, consumers: config.consumers ?? ['.'], packageName: config.designSystem.package, diff, ...(ownership ? { ownership } : {}) }); await output(renderArtifact(report, format), values.output, root); return; }
  if (command === 'plan') { requireArgs(positionals, 3, 'plan requires <diff.json> and <impact.json>.'); const config = await loadConfig(root, values.config); const result = createMigrationPlan(await artifact(positionals[1]!, root, 'design-system-diff'), await artifact(positionals[2]!, root, 'design-system-impact-report'), config.migrations); await output(renderArtifact(result, format), values.output, root); return; }
  if (command === 'check') { requireArgs(positionals, 2, 'check requires <diff.json>.'); const diff = await artifact(positionals[1]!, root, 'design-system-diff'); if (values.consumers) { const config = await loadConfig(root, values.config); const ownership = config.ownership ? await createCodeownersResolver(path.resolve(root, config.ownership.codeowners)) : undefined; const report = await analyzeImpact({ root, consumers: config.consumers ?? ['.'], packageName: config.designSystem.package, diff, ...(ownership ? { ownership } : {}) }); process.stdout.write(renderArtifact(report, format)); if (report.impacts.some((i) => diff.changes.find((c) => c.id === i.changeId)?.severity === 'breaking')) process.exitCode = 1; } else { process.stdout.write(renderArtifact(diff, format)); if (diff.changes.some((c) => c.severity === 'breaking')) process.exitCode = 1; } return; }
...
async function artifact<K extends DesignSystemSnapshot['kind'] | SemanticDiff['kind'] | ImpactReport['kind']>(file: string, root: string, kind: K): Promise<ArtifactByKind<K>> {
  let value: unknown;
  try { value = JSON.parse(await readFile(path.resolve(root, file), 'utf8')) as unknown; }
  catch (error) { throw new ValidationError('DSI1300', `Could not parse artifact ${file}: ${error instanceof Error ? error.message : String(error)}`); }
  return validateArtifact(value, kind);
}
...
main().catch((error: unknown) => { const usage = error instanceof UsageError || error instanceof ValidationError || (error instanceof Error && /^DSI1/.test(error.message)); process.stderr.write(`${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = usage ? 2 : 3; });
