...
function renderImpact(value: ImpactReport, format: Exclude<ReportFormat, 'json'>): string {
  if (format === 'markdown') return `# Consumer impact report\n\n${value.impacts.map((i) => `- \`${i.location.file}:${i.location.line}:${i.location.column}\` — **${i.subject}** affected by \`${i.changeId}\`${i.location.owner ? ` (${i.location.owner.join(', ')})` : ''}`).join('\n') || 'No affected usages found.'}${value.diagnostics.length ? `\n\n## Diagnostics\n\n${value.diagnostics.map((d) => `- **${d.code}**${d.location ? ` \`${d.location.file}:${d.location.line}:${d.location.column}\`` : ''}: ${d.message}`).join('\n')}` : ''}\n`;
  return [`Consumer impact: ${value.package}`, ...value.impacts.map((i) => `${i.location.file}:${i.location.line}:${i.location.column}  ${i.subject}  ← ${i.changeId}${i.location.owner ? `  ${i.location.owner.join(',')}` : ''}`), ...value.diagnostics.map((d) => `${d.severity.toUpperCase()} ${d.code}${d.location ? ` ${d.location.file}:${d.location.line}:${d.location.column}` : ''}  ${d.message}`), `${value.impacts.length} affected usage(s), ${value.diagnostics.length} diagnostic(s)`].join('\n') + '\n';
}
